...
class NIFastCounter(Base, SlowCounterInterface, FastCounterInterface):
    ...
    def get_data_trace(self):
        ...
        # np.int(np.ceil(self.ACQtime / self.period)) # (ACQtime/resolution)/2 #JSS: added (a modified version) in the configure() function

        #tHE following is better placed in start(), as repeated configuring everytime you pull data is NOT RIGHT as QUDI does start()-> get_data_trace(), and youre configuring after starting the clock/counter_task!!
        #'''
        #....Moreover, atleast you should call this next snippet in start(), as PB should start before configuring the DAQ in any case!!!...
        #self.Clock.timing.cfg_samp_clk_timing(1000, source=self.Digital_Channel, active_edge=Edge.RISING,
        #                                     sample_mode=AcquisitionType.FINITE,
        #                                      samps_per_chan=self.samples)  # JSS: Line 81 from s_a.py; with "samps_per_chan" set as "samples";  #JSS: CHECK THIS!! Im not sure what the first argument '1000' does
        #self.reader = CounterReader(self.Clock.in_stream)  # JSS: adapted from Line 83 #JSS:Im calling this in start instead
        #'''
        try:  #JSS: can remove this now
            self.reader.read_many_sample_uint32(self.counts, number_of_samples_per_channel=self.samples, timeout=10000.0)  # JSS: Line 121
        except Exception as e:#JSS: can remove this now
            print("measurement? too early bud:/", e)#JSS: can remove this now
            pass#JSS: can remove this now
        ...
        try:
            ...
            if self.numberofsweeps < 300000 and self.meas_run:  # NI card number of Sweeps
                ...
                #self.start_measure() #JSS: Check this!! this was such a PoS! #JSS: check this!! I think I need this actually #JSS: No we dont need this
