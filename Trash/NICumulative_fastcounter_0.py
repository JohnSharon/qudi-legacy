...
class NIFastCounter(Base, SlowCounterInterface, FastCounterInterface):
    ...
    #_gated = ConfigOption('gated', False, missing='warn') #JSS: added
    ...
    def __init__(self, config, **kwargs):
        ...
        '''
        self.useNIcardDI = 1  ##JSS: I'll remove this attribute later
        ...
        self._count_channel = 1  # JSS: ignore, harmless though useless
    ...
    def start(self, acq_time):
        ...
        if self.useNIcardDI == 1:  # digital
            ...
            try:
                # self.Counter1.StopTask() #JSS: NotNeeded
                # self.Counter1.ClearTask() #JSS: NN
                # self.Counter2.StopTask() #JSS: NN
                # self.Counter2.ClearTask() #JSS: NN
            ...
            # self.Counter1 = daq.Task() #JSS: NN
            # self.Counter2 = daq.Task() #JSS: NN
            self.Clock = nidaqmx.Task()  # daq.Task() #JSS: modified using Line 77 from s_a.py
            print("clock created in start()")
            # read = daq.c_ulong() #JSS: useless in any case
            # read2 = daq.c_uint64() #JSS: NN
            rate = 1000  # JSS: ignored
            n_samples = 1000  # JSS: ignored
            duty_cycle = 0.5  # JSS: ignored
            ...
            # JSS: Check this!: not sure if the following snippet should be placed here Or in "get_data_trace"
            '''
            n_avg = 1000 #JSS: CHECK THIS!!
            self.Clock.timing.cfg_samp_clk_timing(1000, source=self.Digital_Channel, active_edge=Edge.RISING,
                                            sample_mode=AcquisitionType.FINITE, samps_per_chan=n_avg)  #JSS: Line 81 from s_a.py #JSS: CHECK THIS!! Im not sure what the first argument '1000' does
            reader = CounterReader(self.Clock.in_stream) #JSS: Line 83
            counts = np.zeros(n_avg, dtype=np.uint32) #JSS: Line 84
            '''

            #JSS: removing these NN
            '''
    ...
    def stop_device(self):
        ...
        try:
            ...
            print("this is where generation has been stopped before...?",e)

        # self.analog_input2.StopTask() #JSS: NN
        # self.analog_input2.ClearTask() #JSS: NN
    ...
    def configure(self, bin_width_ns, record_length_ns, number_of_gates=0):
        ...
        # self.mybins = np.arange(0, self._record_length_ns * 1e3, self._bin_width_ns * 1e3, dtype='float')  # picosecond #JSS: NN
        ...
        # self.count = int(number_of_gates) #JSS: NN

        # self.mybins[0] = 1e-12 #JSS: NN
        self.firsttimeNI = 1
        self.result = []

        ####################### NI Card
        Resolution = self._bin_width_ns * 1e-9  # it should be in seconds
        # Tm = self._record_length_ns * 1e-9  # it should be in seconds #JSS: NN
        ...
        # self.NumberofSamples = int(np.ceil(Tm / Resolution))   #JSS: NN
        self.Sampling_rate = np.floor(1 / Resolution)
        # self.numSampsPerChan = self.NumberofSamples   #JSS: NN
        # self.Nchannel = 2  #JSS: NN
        # self.myNIdata = np.zeros((self.NumberofSamples * self.Nchannel,), dtype=np.float64) #(Hoda: , seems to be unnecessary)  #JSS: NN
        # self.VoltageMin = 0 #JSS: NN
        # self.VoltageMax = 5 #JSS: NN


        #JSS: Not sure whethere these are best placed here
        samples = int(self.ACQtime * self._num_of_lasers) + 1 #JSS:ADDDED

        try:
            self.Clock.timing.cfg_samp_clk_timing(1000, source=self.Digital_Channel, active_edge=Edge.RISING,
                                            sample_mode=AcquisitionType.FINITE,
                                              samps_per_chan=samples)  # JSS: Line 81 from s_a.py; with "samps_per_chan" set as "samples";  #JSS: CHECK THIS!! Im not sure what the first argument '1000' does
        ...
        self.counts = np.empty(samples, dtype=np.uint32)  # JSS: adapted from Line 84
    ...
    def continue_measure(self):
        ...
        self.meas_run = True
        self.start(
            self._record_length_ns)  # self.start(self._record_length_ns / 1e6)  # /1e6 was here #JSS: modified for "integer-al" self._record_length_ns
    ...
    def get_data_trace(self):
        ...
        if self.useNIcardDI == 1:
            ...
            # n_read_samples = daq.int32() #check this #JSS: NN

            samples = int(self.ACQtime * self._num_of_lasers) + 1  # np.int(np.ceil(self.ACQtime / self.period)) # (ACQtime/resolution)/2 #JSS: modified
            ...
            self.Counter2.ReadCounterU32(2 * samples,
                                         ...
                                         None)  # PFI7
            '''
            #tHE following is better placed in configure(), as repeated configuring everytime you pull data is not right (inefficient ig) ; Source:chatgpt (told me to put it in start())
            #'''
            #self.Clock.timing.cfg_samp_clk_timing(1000, source=self.Digital_Channel, active_edge=Edge.RISING,
            #                                     sample_mode=AcquisitionType.FINITE,
            #                                      samps_per_chan=samples)  # JSS: Line 81 from s_a.py; with "samps_per_chan" set as "samples";  #JSS: CHECK THIS!! Im not sure what the first argument '1000' does
            reader = CounterReader(self.Clock.in_stream)  # JSS: adapted from Line 83
            #'''
            reader.read_many_sample_uint32(self.counts, number_of_samples_per_channel=samples,
                                           timeout=10000.0)  # JSS: Line 121
        ...
        if self.useNIcardDI == 1:
            ...
            # Sync = self.count_data[0, :] #JSS: deleted
            # Laser = self.count_data2[0, :] #JSS: deleted
            # a = np.argwhere(Sync > 0.5) #JSS: deleted

            Laser_cumulative = self.counts  # JSS: added
            ...
            try:
                ...
                for i in range(np.size(a) - 1):
                    if i != int(np.size(a)) - 1:
                        LaserSum[0:int(a[i + 1]) - int(a[i]) + 1] = LaserSum[0:int(a[i + 1]) - int(a[i]) + 1] + Laser[int(a[i]):int(a[i + 1]) + 1]
                '''
                Laser = np.diff(Laser_cumulative)  # JSS: added
                Laser = Laser.reshape(self.ACQtime, self._num_of_lasers) #Laser=np.array_split(Laser, self.ACQtime)  # JSS: added #JSS:deleted
                LaserSum = np.sum(Laser, axis=0)  # JSS: added #JSS:deleted
                print("LaserSum", LaserSum, len(LaserSum))
                if self.firsttimeNI == 1:
                    self.LaserSumhelper = np.zeros(self._num_of_lasers)  # np.zeros(ArraySize + 1) #JSS: Modified
                    self.firsttimeNI = 0
                self.LaserSumhelper[0:np.size(LaserSum)] += LaserSum
    ...
    def stop_measure(self):
        try:
            # self.Counter1.StopTask() #JSS: deleted
            # self.Counter1.ClearTask() #JSS: deleted
            # self.Counter2.StopTask() #JSS: deleted
            # self.Counter2.ClearTask() #JSS: deleted
