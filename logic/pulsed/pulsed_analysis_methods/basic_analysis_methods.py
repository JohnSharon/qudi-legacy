...
class BasicPulseAnalyzer(PulseAnalyzerBase):
    ...
    def analyse_NI_coff_con(self, laser_data): #JSS
        """
        ...
        c_on = LaserSum[1::2]  # post gate readout
    ...
    def analyse_NI_con_coff(self, laser_data): #JSS
        """
        This method will break the laser data into a matrix where each row corresponds to one sweep.
        Then it averages over them to get the mean sweep data, and calculates the contrast from there.
        Need to be used in combination with the extractor: gated_NI_laser_extractor
        ...
        c_on = LaserSum[0::2]  # post gate readout
        c_off = LaserSum[1::2]  # polarisation readout

        c_on_std = LaserSum_std[0::2]
        c_off_std = LaserSum_std[1::2]
