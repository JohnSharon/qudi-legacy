# -*- coding: utf-8 -*-
"""
This file contains the Qudi hardware module for the NIFastcounter that does gated counting and is compatible with pulsed_measurement_logic.
...
class NIFastCounter(Base, SlowCounterInterface, FastCounterInterface):
    ...
    def start(self, acq_time):
        ...
        if self.useNIcard == 1:  # Analog
            ...
            self.analog_input2.CreateAIVoltageChan(self.Sync_Channel, "myChannelai3", daq.DAQmx_Val_Diff,
                                                   self.VoltageMin,
                                                   self.VoltageMax,
    ...
    def configure(self, bin_width_ns, record_length_ns, number_of_gates=0):
        ...
        self.testStatue = 0
        self._bin_width_ns = bin_width_ns * 1e9  # the input is in second I believe and not nanosecond  #JSS: hardcoded to 100us for testing  #JSS: On the GUI this not changing
        ...
        self.Nchannel = 2
        self.myNIdata = np.zeros((self.NumberofSamples * self.Nchannel),
                                 dtype=np.float64)  # (Hoda: , seems to be unnecessary)
        self.VoltageMin = 0
        self.VoltageMax = 5
    ...
    def get_data_trace(self):
        ...
        if self.useNIcard == 1:  # Analog
            ...
            a = np.argwhere(
                Sync > 1.5)  # a column contains of the indexes of sync array's elements which have values above 1.5
            # print(a)
            try:
                ArraySize = np.max(np.diff(np.transpose(a),
                                           1))  # the difference between the indexes of each 2 sync in a row gives us the pulse length.
                LaserSum = np.zeros(
                    ArraySize + 1)  # we add one to the pulse length to compensate any possible mistake in previous steps (specially finding a sync)
                ...
                self.data_trace = self.LaserSumhelper  # ?Hoda: adding all pulses in all sweeps togeather?      #JSS: Exactly my doubt!!!!!!!!!!!!
                self.analog_input2.StopTask()
                self.analog_input2.ClearTask()
        ...
        if self.useNIcardDI == 1:
            ...
            print("Sync:", Sync)  # JSS
            print("Laser:", Laser)  # JSS
            print(a)
            try:
                LaserSum = np.zeros(1)  # Hoda: Akhe Chera? :))
                ArraySize = np.max(np.diff(np.transpose(a), 1))
                LaserSum = np.zeros(np.size(a)-1)  #LaserSum = np.zeros(ArraySize + 1) #JSS: Changed
                for i in range(np.size(a) - 1):
                    if i != int(np.size(a)) - 1:
                        LaserSum[i] = np.sum(Laser[int(a[i]):int(a[i + 1]) + 1]) #LaserSum[0:int(a[i + 1]) - int(a[i]) + 1] += Laser[int(a[i]):int(a[i + 1]) + 1] #JSS: Changed
                if self.firsttimeNI == 1:
                    self.LaserSumhelper = np.zeros(np.size(a)-1) #np.zeros(ArraySize + 1)  #JSS: Changed
                    self.firsttimeNI = 0
                self.LaserSumhelper[0:np.size(LaserSum)] += LaserSum  # self.LaserSumhelper[0:np.size(LaserSum)]=LaserSum+self.LaserSumhelper[0:np.size(LaserSum)]  #JSS: Changed
                self.data_trace = self.LaserSumhelper  # JSS: feel this shld be `self.data_trace.append(self.LaserSumhelper)`
        ...
        print("self.data_trace:", self.data_trace, "self.data_trace_helper:", self.data_trace_helper,
              "self.data_trace_helper20:", self.data_trace_helper20)
    ...
    def start_measure(self):
        ...
        self.start(
            int(self._record_length_ns / 1e6))  # Measurement time in millisec (unit ms) it is acq time which should be between 1 to... ms
        print('start measure2')  # JSS: uncommented
        self.sigReadoutNI.emit()
        print('start measure3')  # JSS: uncommented
