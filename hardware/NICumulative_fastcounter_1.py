...
You use contrast_extractor and pass_through_analyze for this, as the laser difference is already taken care in the module.
...
class NIFastCounter(Base, SlowCounterInterface, FastCounterInterface):
    ...
    def configure(self, bin_width_ns, record_length_ns, number_of_gates=0, stop_sweep=0):
        print('configure')
        self.startSweep = 0
        self.mycounter = 1
    ...
    def get_data_trace(self):
        ...
        try:  #JSS: can remove this now
            self.reader.read_many_sample_uint32(self.counts, number_of_samples_per_channel=self.samples, timeout=300.0)  # JSS: Line 121
        ...
        try:
            ...
            print("sweep number:", self.numberofsweeps)
            self.data_trace = self.LaserSumhelper/self.numberofsweeps
            print("data_trace:", self.data_trace)
    ...
    def start_measure(self):
        ...
        print('start_measure')
        self.numberofsweeps = 1 #JSS: this was already there in stop_measure, but then why is not updating in the gui, so try addding here
